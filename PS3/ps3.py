...
def is_valid_word(word, hand, word_list):
    ...
    lowercase_word = word.lower()
    loop_end = 1
    index_of_wildcard = lowercase_word.find('*') 
    flag = True
    ...
    while loop_counter < loop_end:
        flag = True
        if  loop_end > 1 :
            lowercase_word = lowercase_word[0:index_of_wildcard] + VOWELS[loop_counter] + lowercase_word[index_of_wildcard+1:]
        if word_list.count(lowercase_word) == 0:
            flag = False
        else:   
            new_hand = hand.copy() ##to modify it without modifying the actual hand
            for char in lowercase_word:
                if new_hand.get(char,0) >=1:
                    new_hand[char] -= 1
                elif char == VOWELS[loop_counter]:
                    if new_hand['*'] > 0:
                        del(new_hand['*'])
                else:
                    flag = False
